...
[![Dashboard view of the clay-widgets demo](https://alessandroau.github.io/clay-widgets/screenshots/dashboard.png)](https://alessandroau.github.io/clay-widgets/)
...
| Dashboard | Tasks |
| --- | --- |
| ![Dashboard](https://alessandroau.github.io/clay-widgets/screenshots/dashboard.png) | ![Tasks](https://alessandroau.github.io/clay-widgets/screenshots/tasks.png) |

| Gallery | Settings |
| --- | --- |
| ![Gallery](https://alessandroau.github.io/clay-widgets/screenshots/gallery.png) | ![Settings](https://alessandroau.github.io/clay-widgets/screenshots/settings.png) |

The screenshots are regenerated by CI on every push to `main` and published
with the web demo (see "Screenshot harness" below), so they never drift from
what the demo actually renders.
...
`tools/screenshot_panels.py` drives this harness once per view to capture
every panel into `build/screenshots/` in one command.

### Automated in CI

The `screenshots` job in `.github/workflows/pages.yml` does the same thing on
every push to `main` and deploys the PNGs to GitHub Pages under
`/screenshots/`, next to the web demo, which is where this README loads them
from - so CI never commits to the repo. GitHub runners have no GPU, so it
...
`-lGL -lX11` there instead of `-lopengl32 -lgdi32`.
