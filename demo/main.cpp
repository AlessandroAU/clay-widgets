...
// with no external asset files. The renderer itself is font-agnostic; the demo
// hands these bytes to its FontCache below.
#include "assets/generated/embedded-font.h"

namespace {

// Bridge raylib's clipboard to the widget library so text inputs support
// copy/cut/paste (Ctrl+C/X/V).
const char *GetClipboardTextForWidgets(void *) {
    const char *text = GetClipboardText();
    return text ? text : "";
}

void SetClipboardTextForWidgets(const char *text, void *) {
...
int main(int argc, char **argv) {
    ...
    fontCache.ttfData = kEmbeddedRobotoTTF;
    fontCache.ttfSize = static_cast<int>(kEmbeddedRobotoTTFSize);
    {
    ...
    ClayWidgets_SetClipboardFunctions(&ui, GetClipboardTextForWidgets, SetClipboardTextForWidgets, nullptr);
    ...
    auto frameStep = [&]() {
        ...
        // Editing and navigation keys honor OS key repeat (raylib reports
        // repeats separately), so holding Backspace keeps deleting just like
        // held character keys keep typing.
        auto keyRepeat = [](int key) { return IsKeyPressed(key) || IsKeyPressedRepeat(key); };
        input.keyBackspace = keyRepeat(KEY_BACKSPACE);
        input.keyDelete = keyRepeat(KEY_DELETE);
        input.keyHome = IsKeyPressed(KEY_HOME);
        input.keyEnd = IsKeyPressed(KEY_END);
        input.keyLeft = keyRepeat(KEY_LEFT);
        input.keyRight = keyRepeat(KEY_RIGHT);
        ...
        input.keyPaste = ctrlDown && (IsKeyPressed(KEY_V) || IsKeyPressedRepeat(KEY_V));
