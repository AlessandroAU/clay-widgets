...
#include "test-color-picker.h"

static void TestInvisiblePointerCapture(void) {
    Clay_ElementId id = CLAY_ID("InvisibleCapture");
    bool disabled = false, visible = true, clicked = false;
    auto draw = [&]() {
        if (!visible) return;
        clicked = ClayWidgets__InteractButton(&ui, id, disabled, id).clicked;
        CLAY(id, { .layout = { .sizing = { .width = CLAY_SIZING_FIXED(80), .height = CLAY_SIZING_FIXED(30) } } }) {}
    };
    Frame(MakeInput(), draw);
    Frame(PressAt(10,10), draw);
    CHECK(ui.activeId == id.id);
    ClayWidgets_Input drag = MakeInput(); drag.pointerDown = true; drag.mouseX = 200; drag.mouseY = 100;
    Frame(drag, draw); CHECK(ui.activeId == id.id);
    Frame(ReleaseAt(10,10), draw); CHECK(clicked);
    Frame(PressAt(10,10), draw);
    visible = false; Frame(drag, draw); CHECK(ui.activeId == 0);
    visible = true; Frame(PressAt(10,10), draw);
    disabled = true; Frame(drag, draw); CHECK(ui.activeId == 0);
}
...
int main(void) {
    ...
    const TestCase tests[] = {
        ...
        { "modal sizing and dismissal", TestModalSizingAndDismissal },
        { "picker exact entry", TestColorPickerEntry },
        { "color conversion", TestColorConversion },
        { "color picker dragging", TestColorPickerDrag },
        { "color picker keyboard and palette", TestColorPickerNavigation },
        { "color picker disabled", TestColorPickerDisabled },
        { "color picker dialog", TestColorPickerDialog },
        { "invisible pointer capture", TestInvisiblePointerCapture },
